...
export default function QuevedoCalendar() {
    ...
    <SectionLayout className="mb-20">
      ...
      <section className="flex justify-center items-center flex-col">
        <ul className="flex flex-col gap-4 mb-20">
          {items.map((item) => (
            <QuevedoCalendarCard handleSelect={() => handleSelect(item)} key={item.id} {...item} />
          ))}
        </ul>
...
function QuevedoCalendarCard(item: (typeof items)[number] & { handleSelect: () => void }) {
