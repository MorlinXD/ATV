...
//         <p className='absolute font-marker text-hcadarkblue text-md md:text-xl lg:text-3xl lg:p-8'>{slide.text}</p>
//       </div>
//       ))}
//     </div>
//   )
      ...
      <div className="className='overflow-hidden mt-10 relative">
        {texts.map((e, index) => (
          <div key={index} className={index === activeIndex ? 'block duration-700 ease-in-out' : 'hidden duration-700 ease-in-out'}>
            {/* <img src={text} className="absolute block w-full -translate-x-1/2 -translate-y-1/2 top-1/2 left-1/2" alt={`Slide ${index + 1}`} /> */}
