  ...
  return (
    <div className="grid gap-4 grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
      {galleries.map((gallery, index) => (
        <button key={index} onClick={() => { openModal(index) }} className='relative w-full pb-[75%] focus:outline-none'>
          <Image
            src={gallery.src}
       ...
       {isModalOpen && currentImageIndex !== null && (
        <div className="fixed top-0 left-0 w-full h-full z-50 flex items-center justify-center bg-black bg-opacity-80">
          <button onClick={closeModal} className="absolute top-4 right-4 p-2 text-white focus:outline-none">
            <span className='text-4xl text-white font-marker font-bold'>X</span>
          </button>
          <button onClick={goToPrevImage} className="absolute left-4 p-2 text-white focus:outline-none">
            <span className='text-4xl text-white font-marker font-bold'>{'<'}</span>
          </button>
          <button onClick={goToNextImage} className="absolute right-4 p-2 text-white focus:outline-none">
            <span className='text-4xl text-white font-marker font-bold'>{'>'}</span>
          </button>
          <Image
